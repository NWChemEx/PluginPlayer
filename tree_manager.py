...
#delete the entire tree and its edges and nodes
    ...
    def add_node(self, instance):
        ...
        module_name = self.plugin_player.plugin_manager.saved_plugins[plugin_number].modules[module_number]
        ...
        self.plugin_player.add_message(f"Added new node {module_name} to the tree")
    ...
    def remove_node(self, instance):
        ...
        self.plugin_player.add_message(f"Initiating removal of {node.module_name} Node: {node_number}")
        ...
        for input_edge in node.input_map:
            if input_edge and input_edge[0] != -1:
                ...
                self.plugin_player.add_message(f"Removed Output {output_key} of Node {output_node_number} to {input_key} from Node {node_number}")
            input_number += 1
        ...
        for output in node.output_map:
            for output_edge in output:
                if output_edge:
                    ...
                    self.plugin_player.add_message(f"Removed Input: {input_key} of Node {input_node_number} to {output_key} from Node {node_number}")
            output_number += 1

        #delete data from tree list
        self.plugin_player.nodes[node_number] = None

        self.plugin_player.add_message(f"Removed {node.module_name} Node: {node_number} from the tree")
        return
    ...
    def run_tree(self):
        ...
        for node in nodes:
            ...
            for input in node.input_map:
                if not input:
                    self.plugin_player.add_message(f"Run Failure: {node.module_name}({nodes.index(node)}) has a missing input")
                    return
                if input[0] != -1:
                    has_dependency = True
            ...
            for submod in node.submod_map:
                if not submod[1]:
                    self.plugin_player.add_message(f"Run Failure: {node.module_name}({nodes.index(node)} has missing submodule")
                    return

            #check if property type is set
            if not node.property_type:
                self.plugin_player.add_message(f"Run Failure: {node.module_name}({nodes.index(node)}) has no property type set")
                return
        ...
        if len(dfs_stack) == 0:
            self.plugin_player.add_message("Could not find starting node with no routed inputs")
            return
        ...
        for run_node in dfs_result:
            ...
            for submod_set in run_node.submod_map:
                try:
                    ...
                    self.plugin_player.add_message(f"Couldn't add submodule {submod_set[0]} to {run_node.module_name}")
            ...
            try:
                ...
                self.plugin_player.add_message(f"{run_node.module_name}({nodes.index(run_node)}) Output: {output}")
                saved_outputs[nodes.index(run_node)].append(output)
            except Exception as e:
                self.plugin_player.add_message(f"Could not run {run_node.module_name}({nodes.index(run_node)}): {e}")
                self.plugin_player.add_message(f"Aborting tree run")
                return              
        ...
        for output_array in current_node.output_map:
            for output in output_array:
                ...
                if next_node in visited:
                    ...
                    self.plugin_player.add_message(f"Cycle in tree detected from {next_node.module_name}({next_node_number}) to {current_node.module_name}({nodes.index(current_node)}))")
                    return None
