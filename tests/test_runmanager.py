...
class TestRunManager(unittest.TestCase):
    ...
    def test_add_input(self):
        ...
        self.assertEqual(player.run_manager.module_dict["Multiply by 1"].evaluated_inputs[0], 5, "Did not save the input")
    ...
    def test_link_property_type(self):
        ...
        #check if the first input was set
        self.assertTrue(isinstance(player.run_manager.module_dict["Multiply by 1"].evaluated_property_type, Multiplier), "Did not set property type to Multiplier")
    ...
    def test_link_submod(self):
        ...
        self.assertTrue(player.mm.at("Multiply by Submods").submods()['internal multiplier 1'].has_name(), "Did not set a submodule in the module manager")
        #check that the submodule is set correctly
        self.assertEqual(player.mm.at("Multiply by Submods").submods()['internal multiplier 1'].get_name(), "Multiply by 2", "Did not set the submodule to Multiply by 2")   
    ...
    def test_run(self):
        ...
        player.mm.change_submod("Multiply by Submods", "internal multiplier 1", "Multiply by 2")
        player.mm.change_submod("Multiply by Submods", "internal multiplier 2", "Multiply by 1")
        
        #set input to 10
        player.run_manager.module_dict["Multiply by Submods"].inputs[0] = "5"
        player.run_manager.module_dict["Multiply by Submods"].evaluated_inputs[0] = 5  
        player.mm.at("Multiply by Submods").change_input("r", 5)
   
        
        #Set property type
        player.run_manager.module_dict["Multiply by Submods"].property_type = "Multiplier()"
        player.run_manager.module_dict["Multiply by Submods"].evaluated_property_type = Multiplier()
        ...
        self.assertEqual(last_message_log, "Output: 50", "Incorrect output message, did not run successfully")
