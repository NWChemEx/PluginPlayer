...
class RunManager():
    ...
    def inputs_config(self):
        ...
        if (module_name == None):
            self.plugin_player.add_message(
                "Can't configure inputs: No module added to the tree")
            return
        ...
        for i in range(
                len(self.plugin_player.mm.at(module_name).inputs().items())):
            #grab the key for the input
            key = list(
                self.plugin_player.mm.at(module_name).inputs().keys())[i]
            try:
                value_description = list(
                    self.plugin_player.mm.at(
                        module_name).inputs().values())[i].description()
            except:
        ...
        self.plugin_player.create_popup(
            scroll_view, f'Input Configuration: {module_name})', True,
            (dp(800), dp(400)))
        return
    ...
    def add_input(self, instance):
        ...
        try:
            ...
            self.plugin_player.mm.at(module_name).change_input(
                key, custom_input)

            #if successful add it to the module dictionary to save and add success message
            self.module_dict[module_name].inputs[
                key_number] = custom_declaration
            self.module_dict[module_name].evaluated_inputs[
                key_number] = custom_input

            self.plugin_player.add_message(
                f'Input {key} of {module_name} successfully set as {custom_declaration} in ModuleManager'
            )
    ...
    def property_type_config(self):
        ...
        if (module_name == None):
            self.plugin_player.add_message(
                "Can't configure property type: No module loaded onto the tree"
            )
            return
        ...
        custom_entry_button = Button(text="Set",
                                     height=dp(30),
                                     size_hint_y=None,
                                     size_hint_x=1 / 5,
        ...
        self.plugin_player.create_popup(
            ptype_widget, f'Property Type Configuration: {module_name})', True,
            (dp(450), dp(150)))
        return
    ...
    def add_property_type(self, instance):
        ...
        try:
            ...
            self.module_dict[
                module_name].evaluated_property_type = property_type_eval

            self.plugin_player.add_message(
                f'Property type of {module_name} successfully set as {custom_declaration} in ModuleManager'
            )
    ...
    def submods_config(self, instance):
        ...
        for i in range(len(module.submods().items())):
            ...
            submod_add_button = Button(text='Set',
                                       height=dp(30),
                                       size_hint_y=None,
                                       size_hint_x=1 / 5)
        ...
        #add to the popup
        self.plugin_player.create_popup(
            scroll_view, f'Submodule Configuration for {module_name}', True,
            (dp(800), dp(300)))
    ...
    def select_submod(self, instance):
        ...
        module_name = ' '.join(instance.id.split()[1:])
        key = list(
            self.plugin_player.mm.at(module_name).submods().keys())[key_number]
    ...
    def add_submod(self, instance):
        ...
        if (submodule_name == module_name):
            self.plugin_player.add_message(
                "A module's submodule cannot be itself, create a clone to add it as a submodule."
            )
            fakeButton = Widget()
        ...
        self.plugin_player.tree_manager.add_connections(
            module_tree, tree_nodes)
        fakeButton = Widget()
        fakeButton.id = module_name
        self.submods_config(fakeButton)
    ...
    def run(self):
        ...
        if (module_name == None):
            self.plugin_player.add_message(
                "Run Error: No module loaded onto the tree")
            return
        ...
        for i in range(len(self.module_dict[module_name].inputs)):
            if (self.module_dict[module_name].inputs[i] == None):
                inputs_set = False
                self.plugin_player.add_message(
                    f"Input {list(module.inputs().keys())[i]} not set")
        ...
        if (not module.ready()):
            self.plugin_player.add_message(
                "Module is not ready to run, check all submodules and inputs are mapped"
            )
            return
        ...
        try:
            output = module.run_as(
                self.module_dict[module_name].evaluated_property_type,
                *self.module_dict[module_name].evaluated_inputs)

            #print success message and output
            self.plugin_player.add_message(
                f"Successfully ran {module_name}\nOutput: {output}")
