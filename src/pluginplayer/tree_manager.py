...
class TreeManager():
    ...
    def create_node(self, module_name):
        ...
        with node_widget.canvas.before:
            ...
            rect = RoundedRectangle(size=node_widget.size,
                                    pos=node_widget.pos,
                                    radius=[dp(20),
                                            dp(20),
                                            dp(20),
        ...
        basis_box = BoxLayout(orientation='horizontal',
                              spacing=0,
                              size=(self.node_width, self.node_height),
                              size_hint=(None, None))
        ...
        widget_label = Label(size_hint=(None, None),
                             width=dp(100),
                             height=dp(80),
                             halign='center',
                             valign='center',
        ...
        node_widget.bind(
            pos=lambda instance, value: setattr(basis_box, 'pos', value))
    ...
    def set_module(self, instance):
        ...
        plugin = self.plugin_player.plugin_manager.saved_plugins[int(
            instance.id.split()[0])]
    ...
    def submodule_dependencies(self):
        ...
        while (has_dependency):
            ...
            while (module_index < len(module_tree[tree_layer])):
                ...
                submodule_list = mm.at(
                    module_tree[tree_layer][module_index][1]).submods()
    ...
    def generate_tree(self, module_tree):
        ...
        for layer in module_tree:
            ...
            layer_layout = BoxLayout(orientation='horizontal',
                                     width=0,
                                     size_hint=(None, None),
                                     spacing=dp(50))
        ...
        for layer in layers:
            ...
            position_x = (1 - (layer.width / tree_section.width)) / 2 + (
                layer.width / tree_section.width) / 2
    ...
    def add_connections(self, module_tree, tree_nodes):
        ...
        for module_map_list in module_tree:
            ...
            for module_map in module_map_list:
                ...
                parent_module = tree_nodes[layer_count -
                                           1][parent_module_index]

                # grab the dependent module
                child_module = tree_nodes[layer_count][module_count]

                # Create a Line object
                connecting_line = Line(width=dp(3),
                                       points=[
                                           parent_module.center_x,
                                           parent_module.center_y,
                                           child_module.center_x,
                                           child_module.center_y
                                       ])

                #add to the screen
                tree_section.canvas.before.add(
                    Color(192 / 255, 59 / 255, 20 / 255))  #NWChemEX Red
                ...
                update_line = lambda instance, value, connecting_line=connecting_line, parent_module=parent_module, child_module=child_module: setattr(
                    connecting_line, 'points', [
                        parent_module.center_x, parent_module.center_y,
                        child_module.center_x, child_module.center_y
                    ])
